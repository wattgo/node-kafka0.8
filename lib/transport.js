...
var KafkaTransport = function (options, callback) {
	...
	this.startup = true;
	this.clientId = options.clientId || this.defaultClientId;
...
KafkaTransport.prototype.encodeMessageSet = function(messages, compression, callback) {
	...
	messages.forEach(function(item) {
		var currentItem = (typeof item !== 'string' ? JSON.stringify(item) : item)
		var m = self.encode(self.schema.Message, 0/*crc*/, self.magicByte, compression, 0/*key*/, currentItem);
			...
			case 'gzip': 
				zlib.gzip(set, function(err, gzipped) {
					self.encodeMessageSet([ gzipped ], self.compressions.gzip, function(compressed) {
						//console.log('gzipping', set, set.length, 'to', compressed, compressed.length)
						callback(compressed);
					})
				})
			break;
			case 'snappy':
				var snapped = snappy.compressSync(set);
				self.encodeMessageSet([ snapped ], self.compressions.snappy, function(compressed) {
					//console.log('snappying', set, set.length, 'to', compressed, compressed.length)
					callback(compressed);
				})
			break;
		}
	...
	if(!bin.eof()) {
		...
		switch(msg.Attributes) {
			...
			case this.compressions.snappy:
				var unsnapped = snappy.decompressSync(msg.Value);
				self.decodeMessageSet(unsnapped, callback, end);
			break;
			default:
				callback(msg.Value.toString(), header.Offset, msg);
		}
		this.decodeMessageSet(bin, callback, end);
	}
	else {
		end();
	}
